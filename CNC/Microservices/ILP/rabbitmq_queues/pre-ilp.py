...
while(rabbitmw_available==False):
    try:
        ...
        time.sleep(5)
channel.queue_declare(queue='pre-ilp', durable=True)
print(' [*] Waiting for messages. To exit press CTRL+C')


def callback(ch, method, properties, body):
    with open("/var/preprocessing.txt", "w") as text:
            text.write(body.decode())
    ch.basic_ack(delivery_tag=method.delivery_tag)


channel.basic_qos(prefetch_count=1)
channel.basic_consume(queue='pre-ilp', on_message_callback=callback)
